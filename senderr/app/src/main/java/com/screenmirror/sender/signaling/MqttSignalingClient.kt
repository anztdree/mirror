package com.screenmirror.sender.signaling
...
/**
 * MQTT signaling client for SENDER side with MULTI-BROKER fallback + timeouts.
 */
class MqttSignalingClient(
    ...
    suspend fun connect() = withContext(Dispatchers.IO) {
        ...
        for (broker in BROKERS) {
            try {
                ...
                // Pre-flight DNS check - fail fast if DNS doesn't resolve
                val addresses = try {
                ...
                val c = Mqtt5Client.builder()
                    .identifier("screenmirror_sender_${myPeerId.take(8)}_${broker.host.replace(".", "_")}")
                ...
                val subLobby = Mqtt5Subscription.builder()
                    .topicFilter(lobbyTopic())
                    .qos(MqttQos.AT_LEAST_ONCE)
                    .build()
                val subscribeLobby = Mqtt5Subscribe.builder()
                    .addSubscription(subLobby)
                    .build()
                c.subscribe(subscribeLobby, messageCallback).get(CONNECT_TIMEOUT_SEC, TimeUnit.SECONDS)
    ...
    suspend fun broadcastHello() = withContext(Dispatchers.IO) {
        val c = client ?: throw IllegalStateException("Not connected")
        val json = JSONObject().apply {
            put("type", "sender_ready")
            put("from", myPeerId)
        }
        val payload = json.toString().toByteArray(StandardCharsets.UTF_8)
